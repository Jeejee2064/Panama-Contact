...
const STEPS = [
  ...
  { title: 'Familia / Family', special: 'step3' },
  {
    ...
    fields: [
      ...
      { name: 'telefono_domicilio', label: 'Teléfono de domicilio / Home phone', type: 'text', required: true },
    ],
  },
  {
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'telefono_responsable', label: 'Teléfono del responsable / Guarantor\'s phone', type: 'text', required: true },
      ...
      { name: 'telefono_persona_responsable', label: 'Teléfono de la persona responsable / Phone of responsible person', type: 'text', required: false },
    ],
  },
...
function FieldLabel({ label, required }) {
  return (
    <label className="block text-xs text-gray-500 mb-1 leading-tight">
      {label}
      {!required && <span className="ml-1 text-gray-400">(opcional / optional)</span>}
    </label>
  );
}
...
function inputClass(hasError) {
  return `w-full border rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 transition ${
    hasError ? 'border-red-400 focus:ring-red-300' : 'border-gray-300 focus:ring-orange-400 focus:border-orange-400'
  }`;
}
    ...
    <div className="flex flex-wrap gap-2">
      {options.map((opt) => (
        <label key={opt.value} className="flex items-center cursor-pointer">
          ...
          />
          <span className="px-4 py-2 rounded-full border border-gray-300 text-sm text-gray-700 peer-checked:bg-[#FF491A] peer-checked:border-[#FF491A] peer-checked:text-white transition select-none">
            {opt.label}
          </span>
        </label>
      ))}
...
export default function CuestionarioPage() {
  ...
  const { register, handleSubmit, trigger, watch, formState: { errors } } = useForm({ mode: 'onBlur' });
      ...
      <div className="min-h-screen flex flex-col bg-gray-50">
        ...
        <div className="flex-1 flex items-center justify-center p-8">
          <div className="bg-white rounded-2xl shadow-lg p-10 max-w-lg w-full text-center">
            ...
            <h2 className="text-2xl font-bold text-gray-900 mb-2">¡Enviado con éxito!</h2>
            <p className="text-gray-500 mb-1 text-sm">Su cuestionario ha sido recibido.</p>
            <p className="text-gray-500 text-sm">Your questionnaire has been received.</p>
            <p className="mt-4 text-xs text-gray-400">Nos pondremos en contacto con usted a la brevedad. / We will contact you shortly.</p>
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <div className="px-6 pt-6 pb-2 flex items-center justify-between max-w-2xl mx-auto w-full">
        <span className="text-xs font-semibold text-[#FF491A] uppercase tracking-widest">
          Paso {step + 1} de {totalSteps}
        </span>
        <span className="text-xs text-gray-400">{currentStep.title}</span>
      </div>
            ...
            <div className="flex items-center justify-between mt-6 gap-4">
              {step > 0 ? (
                <button
                  ...
                  className="px-6 py-3 rounded-full border border-gray-300 text-sm font-medium text-gray-700 hover:bg-gray-50 transition"
                >
                  ← Anterior / Back
                </button>
              ) : <div />}
              ...
              {step < totalSteps - 1 ? (
                <button
                  type="button"
                  onClick={goNext}
                  className="px-8 py-3 rounded-full bg-[#FF491A] text-white text-sm font-bold hover:bg-[#e6451a] transition shadow"
                >
                  Siguiente / Next →
                </button>
              ) : (
                <button
                  type="submit"
                  disabled={submitting}
                  className="px-8 py-3 rounded-full bg-[#FF491A] text-white text-sm font-bold hover:bg-[#e6451a] transition shadow disabled:opacity-60 flex items-center gap-2"
                >
                  {submitting && (
